function trapFunctionTightlyLinked(population,offsprings,deceptiveFactor)
    ...
    for i=1:size(population,1)
        for j=1:4:size(population,2)
            tempPopulation=population[i,j:j+3];
            tempOffspring=offsprings[i,j:j+3];
